...
Version corrigée sans erreurs d'indentation
"""
...
def check_no_scan_operations(project_root):
        ...
        for py_file in dir_path.rglob("*.py"):
            ...
            try:
                with open(py_file, 'r', encoding='utf-8') as f:
                    ...
                    # Ignorer les lignes contenant des patterns de vérification
                    if (('#' in line and '.scan' in line) or
                        ('r\'' in line and '.scan' in line) or
                        ('# .scan' in line) or
                        ('patterns' in line.lower() and '.scan' in line) or
                        ('regex' in line.lower() and '.scan' in line)):
                        continue  # Ignorer les définitions de patterns et commentaires
                    
                    # Ignorer les scripts de vérification eux-mêmes
                    if 'check_dynamodb' in py_file.name or 'verify_no_scan' in py_file.name:
                        continue
    ...
    if scan_found:
        print("❌ ÉCHEC: Opérations scan détectées!")
        print("⚠️ Action requise: Remplacer les opérations scan par des opérations query")
        return False
    else:
        print("✅ SUCCÈS: Aucune opération scan détectée!")
...
def check_efficient_operations(project_root):
    ...
    if operations_found:
        print("✅ Opérations efficaces détectées:")
        ...
        print("⚠️ Aucune opération DynamoDB détectée")
...
def main():
    ...
    if no_scan and efficient_ops:
        ...
        print("   ✅ Aucune opération scan utilisée")
