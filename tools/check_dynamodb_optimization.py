#!/usr/bin/env python3
"""
Script de        # Parcourir tous les fichiers Python
        for py_file in dir_path.rglob("*.py"):
            ...
            try:
                ...
                for line_num, line in enumerate(content.split('\n'), 1):
                    # Ignorer les commentaires et les chaînes de caractères dans les patterns
                    if '#' in line and any(pattern.replace('\\', '') in line for pattern in scan_patterns):
                        continue  # Ignorer les lignes de commentaires avec des patterns
                    
                    for pattern in scan_patterns:
                        if re.search(pattern, line, re.IGNORECASE):
                            print(f"❌ SCAN DÉTECTÉ dans {py_file.relative_to(project_root)}:{line_num}")
                            print(f"   Ligne: {line.strip()}")
                            scan_found = TruenamoDB - S'assure qu'aucune opération scan n'est utilisée
"""
